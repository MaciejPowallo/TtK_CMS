<?php include("../../init.php"); ?>
<!DOCTYPE html>
<html>
<head>
	<meta charset=utf-8>
	<meta name=description content="">
	<meta name=viewport content="width=device-width, initial-scale=1">
	<link rel="stylesheet" type="text/css" href="<?php echo APP_RES; ?>bootstrap/css/bootstrap.min.css">
	<link rel="stylesheet" type="text/css" href="<?php echo APP_RES; ?>font-awesome/css/font-awesome.min.css">
	<link rel="stylesheet" type="text/css" href="<?php echo APP_RES; ?>css/main-style.min.css">
	<title>Nieznany błąd</title>
</head>
<body>
<div class="container">
	<div class="panel panel-danger centered text-center offset-top">
	  <div class="panel-heading"><h3><i class="fa fa-bug fa-2x" aria-hidden="true"></i> Nieznany błąd <i class="fa fa-bug" aria-hidden="true"></i></h3></div>
	  <div class="panel-body">Wystąpił nieznany błąd odśwież stronę lub skontaktuj się z administratorem jeśli błąd wystąpi ponownie!!!<i class="fa fa-bug" aria-hidden="true"></i></div>
	</div>
</div>
</body>
</html>