...
// Carousel-------------------------------------------------------------------------------------
	function changeCarouselImage($file, $id_image){
		global $CMS;
		if($stmt = $CMS->Database->query("UPDATE carousel_image SET name = '$file' WHERE id_image = '$id_image'")){
			return TRUE;
		}
		else{
			return FALSE;
	...
	function selectCarouselImage($id_image){
		global $CMS;
		if($result = $CMS->Database->query("SELECT * FROM carousel_image WHERE id_image = '$id_image'")){
